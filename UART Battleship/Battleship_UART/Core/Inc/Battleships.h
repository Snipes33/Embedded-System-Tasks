...
void encodeBoardMessage(uint8_t* sendMessage, uint8_t grid[10][10], CRC_HandleTypeDef* CrcHandle);

void bitMapdecoder(uint8_t *pointer, uint8_t matrix[10][10]);
void decodeShots(uint8_t* message, uint8_t grid[10][10]);
uint8_t decodeBoardMessage(uint8_t message[], uint8_t grid[10][10], CRC_HandleTypeDef* CrcHandle);
uint8_t decodeShipOrigin(uint8_t matrix[10][10],uint8_t message[30],CRC_HandleTypeDef* CrcHandle);
